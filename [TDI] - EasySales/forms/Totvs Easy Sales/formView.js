var FormView = {
	...
	binding: function(){
		...
		$(document).on('click', '.form-radio-offer', FormDaoOffer.setAksGroupsValue);
...
var FormViewOffer = {
	...
	loadAsksGroupsTab: function(models){
		...
		if(CURRENT_STATE == Activity.PREENCHER_QUESTIONARIO || CURRENT_STATE == Activity.MODIFICAR || CURRENT_STATE == Activity.FIM){
			...
			$("#esOfertas :input").attr("disabled", true);
		}
	}
};
...
var FormViewAsks = {
	...
	getTemp: function(ask){		
		...
		var askCodigo= ask.codigo;
		askCodigo = askCodigo.replace(".", "___");
		var temp ='<div class="form-group row '+classHidden+'" id="row_'+askCodigo+'" ><label class="col-md-6 col-sm-6 col-xs-12 control-label text-right" title="'+ask.obs+'">'+$.trim(ask.nivel)+' - '+$.trim(ask.nome)+'</label>';
		...
		for(var i=0; i<switchs.length; i++){
			...
			for(var j=0; j<asks.length; j++){
				...
				else if(ask.tipo == "4")
				{
					...
					ask.tipo = "<input name='ask_"+askCodigo+"' class='checkDad field-filter form-control "+requiredClass+"' id='ask_"+askCodigo+"' type='text'/>";
				}
				asksArray.push(ask);
			}
