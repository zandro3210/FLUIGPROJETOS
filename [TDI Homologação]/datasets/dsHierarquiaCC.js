function createDataset(fields, constraints, sortFields) {
	...
	try {
	    ...
	    var codccusto = "603310103";
	    var email = "graziella.ribeiro@totvs.com.br";
	
	    if ((constraints != null) && (constraints.length > 0)) {
			codccusto = constraints[0].getInitialValue();
			if (constraints.length > 1)
				email = constraints[1].getInitialValue();
		}
	
	    log.info ("++ DATASET DSHIERARQUIA - ccusto:" + codccusto);
	    ...
	    if (codccusto != ""){
			...
			//var xmlParamsValue = "<PARAM><CODCCUSTO>" + codccusto + "</CODCCUSTO></PARAM>";
	        ...
	        // -> validar pelo CC e pelo email do campo Email interno
	        for each(var elemento in DocumentElement.Row) {
	        	//if (elemento.EMAIL_PARTICIPANTE.toString().equals(email)){
