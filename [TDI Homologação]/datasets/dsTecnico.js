function createDataset(fields, constraints, sortFields) {
	var unity = getConstraintValue(constraints, "unidade");
	
	if(unity == null) return null;
	
	var dataset = DatasetBuilder.newDataset();
	var urlWithParams = "https://wscorp.totvs.com.br/TCRMS022D?cUnidade="+unity;
	var object = getDatasetRest(urlWithParams);
	
	dataset.addColumn("CODIGO");
	dataset.addColumn("NOME");
	
	for(var i=0; i<object.TECNICOS.length; i++){
		var row = [];
		row.push(object.TECNICOS[i].CODIGO);
		row.push(object.TECNICOS[i].NOME);
